def check_keys(scenarios):
	...
	for scenario in scenarios.keys():
		if scenario == "normal":
			if "sender" not in scenarios[scenario].keys():
				errors.append(f"Missing 'sender' key for {scenario} scenario")
...
def check_scenarios_names(scenarios_names): 
	...
	for scenario in scenarios_names:
		if (
			not scenario[0].isupper() and 
			scenario != scenario.lower() and 
			scenario != scenario.upper() and 
			"_" not in scenario):
...
def check_strategies(strategies, scenarios):
	...
	for i in range(len(strategies_formatted)):
		...
		if strategies_formatted[i].isupper() and strategies_formatted[i] not in keywords:
			errors.append(f"Inappropriate use of uppercase word {strategies_formatted[i]}")
